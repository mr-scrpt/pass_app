...
class IconTestWindow(QMainWindow):
    def __init__(self):
        ...
        custom_group = self._create_custom_icon_group(
            "Custom Keyboard Icons (Transformed)",
            [
        ...
        fa_group = self._create_icon_group(
        ...
        # Unicode Symbols
        unicode_group = self._create_unicode_group(
            "Unicode Symbols",
            [
                ("↵", "Enter/Return"),
                ("⏎", "Return"),
...
<li><b>Phosphor Icons</b> - https://phosphoricons.com/
   <br>Excellent keyboard icon collection</li>
        ...
        info_label.setStyleSheet(
            f"background-color: rgba(137, 180, 250, 0.1); padding: 15px; border-radius: 8px; color: {extra['primaryTextColor']};"
        )
        ...
        for key_name, label_text, color in icons:
            ...
            try:
                ...
                btn_medium.setStyleSheet(
                    f"border: 2px solid {color}; border-radius: 6px; background-color: rgba{tuple(int(color[i : i + 2], 16) for i in (1, 3, 5)) + (0.15,)};"
                )
                row_layout.addWidget(btn_medium)
                ...
                btn_large.setStyleSheet(
                    f"border: 3px solid {color}; border-radius: 8px; background-color: rgba{tuple(int(color[i : i + 2], 16) for i in (1, 3, 5)) + (0.2,)};"
                )
                row_layout.addWidget(btn_large)
    ...
    def _create_unicode_group(self, title, symbols):
        ...
        for symbol, label_text in symbols:
            ...
            lbl_small.setStyleSheet(
                f"color: {extra['primaryColor']}; background-color: rgba(137, 180, 250, 0.15); border: 2px solid {extra['primaryColor']}; border-radius: 6px;"
            )
            ...
            lbl_medium.setStyleSheet(
                "color: #a6e3a1; background-color: rgba(166, 227, 161, 0.15); border: 2px solid #a6e3a1; border-radius: 6px;"
            )
            ...
            lbl_large.setStyleSheet(
                "color: #f38ba8; background-color: rgba(243, 139, 168, 0.15); border: 2px solid #f38ba8; border-radius: 6px;"
            )
